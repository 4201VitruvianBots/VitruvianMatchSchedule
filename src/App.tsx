        ...
        getTeamMatches(teamNumber, eventKey, apiKey).then((matches: MatchInfo[]) => {
            ...
            const matchElements = matches.map((match: MatchInfo, index, array) =>
                <>
                    ...
                    {generateMatchTransitions(match, array[index+1]).map((text) => (<p className="text-2xl text-black theme-gdark:text-white theme-vdark:text-white">{text}</p>))}
                </>);
                ...
                <div className="p-8 gap-8 grid grid-cols-[1fr_auto_1fr] min-h-0 bg-white theme-gdark:bg-gray-900 theme-vdark:bg-gray-900">
                    ...
                    <div className="text-3xl text-center space-y-5 mx-auto ">
                        {teamMatchInfos.length > 0 &&
                            ...
                            <>
                                <p className="font-bold text-black theme-gdark:text-white theme-vdark:text-white">{teamMatchInfos.find((match) => match.matchStart && match.matchStart > new Date())?.matchName}</p>
